...
route.use = (...middlewares)=> {
    for (let m of middlewares) {
        if (route.globalMiddlewares.indexOf(m) === -1 && typeof (m) === "function") {
            route.globalMiddlewares.push(m)
        }
    }
}

route.group = (prefix, callback, ...middlewares)=> {
    route.groupStack.push({ prefix: prefix, middlewares: middlewares })
    callback()
